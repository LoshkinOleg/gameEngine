...
in VS_OUT // w = world, l = light, t = tangent
{
	vec4 w_FragPos;
	vec4 l_FragPos;
    vec4 t_FragPos;
...
const vec3 lightColor = vec3(150.0f);
// ----------------------------------------------------------------------------
...
{		
    ...
    vec3 V = normalize(fs_in.t_ViewPos.rgb - fs_in.t_FragPos.rgb);
    ...
    for(int i = 0; i < 1; ++i) 
    {
        ...
        vec3 L = normalize(-fs_in.t_LightDir);
